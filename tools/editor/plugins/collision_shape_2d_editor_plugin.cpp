...
void CollisionShape2DEditor::_get_current_shape_type() {
	...
	if (cast_to<CapsuleShape2D>(s)) {
		shape_type = CAPSULE_SHAPE;
	} else if (cast_to<CircleShape2D>(s)) {
		shape_type = CIRCLE_SHAPE;
	} else if (cast_to<ConcavePolygonShape2D>(s)) {
		shape_type = CONCAVE_POLYGON_SHAPE;
	} else if (cast_to<ConvexPolygonShape2D>(s)) {
		shape_type = CONVEX_POLYGON_SHAPE;
	} else if (cast_to<LineShape2D>(s)) {
		shape_type = LINE_SHAPE;
	} else if (cast_to<RayShape2D>(s)) {
		shape_type = RAY_SHAPE;
	} else if (cast_to<RectangleShape2D>(s)) {
		shape_type = RECTANGLE_SHAPE;
	} else if (cast_to<SegmentShape2D>(s)) {
...
void CollisionShape2DEditor::edit(Node* p_node) {
	...
	if (p_node) {
		node=cast_to<CollisionShape2D>(p_node);
...
void CollisionShape2DEditorPlugin::edit(Object* p_obj) {
	...
	collision_shape_2d_editor->edit(cast_to<Node>(p_obj));
}
